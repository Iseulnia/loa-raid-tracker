...
- 업로드 경로에 한글(레이드 이름)을 그대로 넣었다가 Supabase Storage 키 검증에서 거부(Invalid key)당한 적
  ...
  항상 주의)
- **캐릭터 자동 인식 — 이미지 매칭 → OCR로 전환함**: 처음엔 캐릭터별로 이름표 이미지를 등록해서 픽셀
  매칭하는 방식(`templateMatch.ts` 재사용)으로 만들었는데, 사용자가 "이미지 등록 없이 그 자리 글자를 그냥
  읽어오면 안 되냐"고 요청해서 **OCR(tesseract.js, 한국어 traineddata)로 전면 교체함** (`src/lib/ocrMatch.ts`).
  이제 이름이 뜨는 화면 자리를 **딱 한 번만** 등록해두면(캐릭터별 등록 불필요), 스캔 시작 시 그 영역 글자를
  OCR로 읽어서(`recognizeRegionText`) 내 캐릭터 이름 목록과 편집거리 기준으로 매칭함(`matchCharacterName`).
  트레이드오프: 이미지 매칭보다 느리고(첫 실행 시 워커 초기화+한국어 데이터 다운로드 몇 초) 로아 특유의
  장식체 폰트에서 오인식 가능성 있음 — 실사용 피드백 아직 없음. `character_name` 템플릿의 `character_id`
  컬럼은 이제 항상 null로 저장됨(레거시 행엔 남아있을 수 있음, 표시에만 씀). OCR 영역은 "내가 캡처한 것만"
  쓰도록 `created_by`로 한정함(다른 사람 화면 배치/해상도가 다를 수 있어서 — RLS상 다른 사람 것도 보이는
  테이블이라 필터링 안 하면 섞여 들어옴)
- 자동 인식은 선택 사항 — 안 등록해도 수동 드롭다운으로 평소처럼 동작하고, 스캔 중 드롭다운을 직접 바꾸면
  그 뒤로는 자동 인식이 덮어쓰지 않음
...
- **한 스캔 세션에서 여러 캐릭터 연속 처리 지원**: 처음엔 OCR이 스캔 시작 시 딱 한 번만 돌아서, 게임에서
  캐릭터를 바꾸면 스캔을 껐다 켜야 했음(사용자 피드백) → OCR을 4초 간격으로 계속 재시도하도록 바꿔서, 스캔
  중간에 캐릭터를 전환해도 자동으로 따라감. `foundKeysRef`/`found` 목록도 `raidLabel` 단독 키에서
  `characterId:raidLabel` 복합 키로 바꿔서, 캐릭터를 전환해도 이전 캐릭터의 처리 상태와 안 섞이고 나중에
  같은 캐릭터로 돌아와도 중복 처리 안 됨. 목록에도 캐릭터 이름을 같이 표시함 ("키츠네아 · 벨가르딘")
- setRaidCheck으로 체크된 항목엔 취소 버튼 추가 (오탐 대응). 취소해도 게임 화면 배지는 그대로 "참여
  완료"라서, 같은 세션 동안은 재감지 대상에서 제외해 곧바로 다시 체크되는 걸 막음. 레이드 이름만 인식되고
  아직 배지가 안 뜬 상태도 "클리어 X"로 목록에 표시해서 스캔이 제대로 보고 있는지 확인 가능하게 함
