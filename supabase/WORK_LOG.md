...
- **캐릭터 카드 순서를 직접 바꿀 수 있음** — "캐릭터 순서 변경" 버튼 → 팝업(`CharacterReorderModal.tsx`)에서
  드래그로 재배열 → `reorderCharacters` 액션으로 `characters.sort_order`를 배열 인덱스로 일괄 저장. 팝업은
  원정대별로 묶어서 보여줌(안 그러면 여러 원정대 캐릭터가 섞여서 찾기 힘들다는 피드백). **원정대 안에서의**
  카드 순서만 `sort_order` 기준으로 바뀌고, **어느 원정대가 먼저 오는지(본계>부계>부부계)는 예전처럼
  아이템레벨 기준 그대로 유지** — 처음엔 원정대 그룹 순서까지 sort_order로 바꿨다가 "본계가 밑으로 내려갔다"는
  피드백을 받고 원복함
