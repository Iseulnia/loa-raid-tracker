...
- **캐릭터 카드 순서를 직접 바꿀 수 있음** — "캐릭터 순서 변경" 버튼 → 팝업(`CharacterReorderModal.tsx`)에서
  드래그로 재배열 → `reorderCharacters` 액션으로 `characters.sort_order`를 배열 인덱스로 일괄 저장. 카드
  정렬 기준을 기존 아이템레벨 내림차순 → `sort_order` 오름차순으로 바꿨고, 원정대 그룹 순서도 그 안의
  최소 `sort_order` 기준으로 바뀜(전에는 그룹 내 최대 아이템레벨 기준이었음)
- **내 레이드별 현황** 카드 신설 — 레이드 이름별(난이도 무관)로 "3/6" 식 완료 캐릭터 수 표시. 대시보드/공격대
  둘 다에서 항상 "내" 캐릭터 기준으로 보임(모드와 무관)
