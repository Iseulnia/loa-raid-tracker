...
// 일부 레이드는 화면에 난이도가 "노말"이 아니라 "싱글 모드" 또는 "매칭 모드"로 표시되는데(사용자 확인:
// 둘 다 컨텐츠 자체는 노말과 동일), DB의 난이도 값은 그대로 "노말"이라 OCR 텍스트만 별칭으로 같이
// 인정해준다. "매칭 모드"는 "매칭"까지만 별칭으로 잡는다 — 뒤쪽 "모드"가 OCR로 깨져도 인식되게 하려는
// 것이고, 다른 난이도 값이나 레이드 이름(벨가르딘/성당/세르카/종막/4막) 중에 "매칭"이 들어가는 게 없어서
// 이렇게 짧게 잡아도 오탐 위험이 없다.
