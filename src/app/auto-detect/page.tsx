...
export default async function AutoDetectPage() {
  ...
  const [{ data: raids }, { data: templates }, { data: characters }, { data: checks }] = await Promise.all([
    ...
    supabase
      ...
      .eq("template_type", "result_screen_ocr")
  ...
  const resultScreenOcrRegion = templatesWithUrls.find((t) => t.crop && t.created_by === user.id)?.crop ?? null;
  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-6">
      ...
      <section>
        ...
        <ScreenCapture raids={raids ?? []} initialTemplates={templatesWithUrls} allowedTypes={["result_screen_ocr"]} />
      </section>
    </main>
  );
}
